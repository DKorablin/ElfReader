...
namespace AlphaOmega.Debug
{
	...
	public struct Elf
	{
		...
		public enum ELFCLASS : Byte
		{
			...
			NONE = 0,
			/// <summary>Supports machines with 32-bit architectures</summary>
			/// <remarks>It uses the basic types defined in the table labeled "32-Bit Data Types."</remarks>
			CLASS32 = 1,
			/// <summary>Supports machines with 64-bit architectures</summary>
			/// <remarks>It uses the basic types defined in the table labeled "64-Bit Data Types."</remarks>
			CLASS64 = 2,
		}
		...
		public enum SHN : UInt16
		{
			...
			/// For example, a symbol "defined" relative to section number <see cref="SHN.UNDEF"/> is an undefined symbol.
			/// </summary>
			UNDEF = 0,
			...
			/// For example, symbols defined relative to section number <see cref="SHN.ABS"/> have absolute values and are not affected by relocation.
			/// </summary>
			ABS = 0xfff1,
		...
		public enum SHT : UInt32
		{
			...
			/// <remarks>Programs that contain a section of this type do not conform to the ABI</remarks>
			SHLIB = 10,
			/// <summary>
			/// These sections hold a symbol table.
			/// Typically a <see cref="SHT.SYMTAB"/> section provides symbols for link-editing.
			...
			LOUSER = 0x80000000,
			/// <summary>This value specifies the upper boundary of the range of indexes reserved for application programs</summary>
			/// <remarks>Section types between <see cref="SHT.LOUSER"/> and <see cref="SHT.HIUSER"/> can be used by the application, without conflicting with current or future system-defined section types</remarks>
			HIUSER = 0xffffffff,
		}
		...
		/// The ELF header's <see cref="Elf32_Ehdr.e_shoff"/> member gives the byte offset from the beginning of the file to the section header table.
		/// <see cref="Elf64_Ehdr.e_shnum"/> normally tells how many entries the section header table contains.
		/// <see cref="Elf32_Ehdr.e_shentsize"/> gives the size in bytes of each entry.
		/// </summary>
		/// <remarks>
		/// Some section header table indexes are reserved in contexts where index size is restricted, for example, the <see cref="Elf32_Sym.st_shndx"/> member of a symbol table entry and the <see cref="Elf64_Ehdr.e_shnum"/> and <see cref="Elf32_Ehdr.e_shstrndx"/> members of the ELF header.
		...
		public struct Elf32_Shdr
		{
			...
			/// Section type <see cref="SHT.NOBITS"/>, described below, occupies no space in the file, and its sh_offset member locates the conceptual placement in the file.
			/// </summary>
			public UInt32 sh_offset;
			/// <summary>
			/// This member gives the section's size in bytes.
			/// Unless the section type is <see cref="SHT.NOBITS"/>, the section occupies sh_size bytes in the file.
			/// A section of type <see cref="SHT.NOBITS"/> can have a nonzero size, but it occupies no space in the file.
			/// </summary>
		...
		/// The section header table is an array of <see cref="Elf32_Shdr"/> or <see cref="Elf64_Shdr"/> structures as described below.
		/// A section header table index is a subscript into this array.
		/// The ELF header's <see cref="Elf64_Ehdr.e_shoff"/> member gives the byte offset from the beginning of the file to the section header table.
		/// <see cref="Elf64_Ehdr.e_shnum"/> normally tells how many entries the section header table contains.
		...
		public enum STV : Byte
		{
			...
			/// </summary>
			/// <remarks>An internal symbol contained in a relocatable object must be either removed or converted to <see cref="STB.LOCAL"/> binding by the link-editor when the relocatable object is included in an executable file or shared object</remarks>
			INTERNAL = 1,
			/// <summary>
			...
			/// </summary>
			/// <remarks>A hidden symbol contained in a relocatable object must be either removed or converted to <see cref="STB.LOCAL"/> binding by the link-editor when the relocatable object is included in an executable file or shared object</remarks>
			HIDDEN = 2,
			/// <summary>
			...
			/// A symbol with <see cref="STB.LOCAL"/> binding may not have <see cref="STV.PROTECTED"/> visibility.
			/// If a symbol definition with <see cref="STV.PROTECTED"/> visibility from a shared object is taken as resolving a reference from an executable or another shared object, the <see cref="SHN.UNDEF"/> symbol table entry created has <see cref="STV.DEFAULT"/> visibility.
			/// </summary>
			PROTECTED = 3,
		}
		...
		public enum STB : Byte
		{
			...
			LOCAL = 0,
			/// <summary>Global symbols are visible to all object files being combined</summary>
			/// <remarks>One file's definition of a global symbol will satisfy another file's undefined reference to the same global symbol</remarks>
			GLOBAL = 1,
			...
			HIOS = 12,
			/// <summary>Values in this inclusive range are reserved for processor-specific semantics</summary>
			/// <remarks>If meanings are specified, the processor supplement explains them</remarks>
			LOPROC = 13,
			/// <summary>Values in this inclusive range are reserved for processor-specific semantics</summary>
			/// <remarks>If meanings are specified, the processor supplement explains them</remarks>
			HIPROC = 15,
		...
		public enum STT : Byte
		{
			...
			/// <remarks>
			/// Function symbols (those with type <see cref="STT.FUNC"/>) in shared object files have special significance.
			...
			/// Shared object symbols with types other than <see cref="STT.FUNC"/> will not be referenced automatically through the procedure linkage table.
			/// </remarks>
			FUNC = 2,
			/// <summary>The symbol is associated with a section. Symbol table entries of this type exist primarily for relocation and normally have <see cref="STB.LOCAL"/> binding</summary>
			SECTION = 3,
			/// <summary>
			/// Conventionally, the symbol's name gives the name of the source file associated with the object file.
			/// A file symbol has <see cref="STB.LOCAL"/> binding, its section index is <see cref="SHN.ABS"/>, and it precedes the other <see cref="STB.LOCAL"/> symbols for the file, if it is present.
			/// </summary>
			FILE = 4,
			/// <summary>The symbol labels an uninitialized common block</summary>
			/// <remarks>
			/// Symbols with type <see cref="STT.COMMON"/> label uninitialized common blocks.
			/// In relocatable objects, these symbols are not allocated and must have the special section index <see cref="SHN.COMMON"/> (see below).
			/// In shared objects and executables these symbols must be allocated to some section in the defining object.
			/// 
			/// In relocatable objects, symbols with type <see cref="STT.COMMON"/> are treated just as other symbols with index <see cref="SHN.COMMON"/>.
			/// If the link-editor allocates space for the <see cref="SHN.COMMON"/> symbol in an output section of the object it is producing, it must preserve the type of the output symbol as STT_COMMON.
			/// 
			/// When the dynamic linker encounters a reference to a symbol that resolves to a definition of type <see cref="STT.COMMON"/>, it may (but is not required to) change its symbol resolution rules as follows: instead of binding the reference to the first symbol found with the given name, the dynamic linker searches for the first symbol with that name with type other than <see cref="STT.COMMON"/>.
			/// If no such symbol is found, it looks for the <see cref="STT.COMMON"/> definition of that name that has the largest size.
			/// </remarks>
			COMMON = 5,
			/// <summary>Values in this inclusive range are reserved for operating system-specific semantics</summary>
			LOOS = 10,
		...
		/// <summary>An object file's symbol table holds information needed to locate and relocate a program's symbolic definitions and references</summary>
		/// <remarks>
		...
		public struct Elf32_Sym
		{
			...
			/// <summary>This member gives the value of the associated symbol</summary>
			/// <remarks>Depending on the context, this may be an absolute value, an address, and so on</remarks>
			...
			/// </summary>
			/// <remarks>
			/// If this member contains <see cref="SHN.XINDEX"/>, then the actual section header index is too large to fit in this field.
			/// The actual value is contained in the associated section of type <see cref="SHT.SYMTAB_SHNDX"/>.
			/// </remarks>
		...
		public struct Elf64_Sym
		{
			...
			/// <summary>Many symbols have associated sizes</summary>
			/// <remarks>
			/// For example, a data object's size is the number of bytes contained in the object.
		...
		public struct Elf64_Rel
		{
			...
			/// <remarks>
			/// For example, a call instruction's relocation entry will hold the symbol table index of the function being called.
			/// If the index is <see cref="SHN.UNDEF"/>, the undefined symbol index, the relocation uses 0 as the symbol value.
			/// </remarks>
